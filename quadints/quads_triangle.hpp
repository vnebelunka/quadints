...
struct barycentric_triangle {
    ...
    constexpr barycentric_triangle(Scalar x, Scalar y, Scalar z)
        : coords{x, y} {
...
template <typename Scalar>
constexpr barycentric_triangle<Scalar> operator+(
...
template <typename Scalar>
constexpr barycentric_triangle<Scalar>& operator+=(
    barycentric_triangle<Scalar>& c, const barycentric_direction<Scalar>& d) {
    c.coords[0] += d.dirs[0];
    c.coords[1] += d.dirs[1];
    return c;
}
